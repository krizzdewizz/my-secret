...
export type Failure = { outcome: 'error', error: any, value: never } & OrThrow<never>;
export type FailureWithMoreInfo<T> = { outcome: 'error', error: string, value: never, moreInfo: T } & OrThrow<never>;
...
export const error = <T, V = unknown>(error: string, moreInfo?: V): FailureWithMoreInfo<V> => {
    return new class {
